...
import {
  FaArrowLeft, FaCalendarAlt, FaCheckCircle, FaClock,
  FaDumbbell, FaEdit, FaPlay, FaTrash, FaPlus
} from "react-icons/fa";
...
const WorkoutPlanDetail = () => {
  ...
  const [editData, setEditData] = useState({
    name: "", description: "", category: "", difficulty: "", duration: "", frequency: "",
  });
  ...
  const finishWorkout = async () => {
    ...
    try {
      await axiosInstance.post("/workout-sessions", {
        ...
        duration: parseInt(plan.duration) || 45,
      });
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
        <div>
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            <div className="flex items-center"><FaDumbbell className="text-[#28A745] mr-2" /><span className="text-sm text-gray-600">Difficulty: {plan.difficulty}</span></div>
            <div className="flex items-center"><FaClock className="text-[#28A745] mr-2" /><span className="text-sm text-gray-600">{plan.duration} min</span></div>
            <div className="flex items-center"><FaCalendarAlt className="text-[#28A745] mr-2" /><span className="text-sm text-gray-600">{plan.frequency}</span></div>
          </div>
        </div>

        <div className="flex gap-3">
          {isStarted ? (
            <button onClick={finishWorkout} className="flex items-center gap-2 bg-[#28A745] text-white px-4 py-2 rounded-lg hover:bg-[#218838]">
              <FaCheckCircle /> Finish Workout
            </button>
          ) : (
            <button onClick={startWorkout} className="flex items-center gap-2 bg-[#28A745] text-white px-4 py-2 rounded-lg hover:bg-[#218838]">
      ...
      {isStarted && (
        <div className="mt-6 p-6 bg-green-50 border-l-4 border-green-500 text-green-700 rounded-lg shadow-sm animate-pulse">
          ...
          <p>Stay strong! You are currently doing: <strong>{plan.name}</strong></p>
            ...
            <ul className="space-y-2">
              {plan.exercises.map((exercise, idx) => (
                <li key={idx} className="flex items-center gap-2">
                  <input type="checkbox" checked={completedExercises[idx] || false} onChange={() => toggleExerciseComplete(idx)} className="h-4 w-4" />
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-4 mt-4">
                ...
                <button type="submit" className="px-4 py-2 bg-[#28A745] text-white rounded-lg hover:bg-[#218838]">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleAddExerciseSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-4 mt-4">
                <button type="button" onClick={closeModal} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-[#28A745] text-white rounded-lg hover:bg-[#218838]">Add Exercise</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="flex justify-end gap-4 mt-6">
              <button onClick={closeModal} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
